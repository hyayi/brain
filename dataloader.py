...
class BrainDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage = None):
        ...
        if stage == 'fit' or stage is None:
            self.train_ds = ImageDataset(image_files=self.train['image'], labels=self.train['label'], transform=train_transform,image_only=False,transform_with_metadata=True)
            self.validation_ds = ImageDataset(image_files=self.val['image'], labels=self.val['label'], transform=val_transform,image_only=False,transform_with_metadata=True)
        if stage == 'test' or stage is None:
            self.test_ds = ImageDataset(image_files=self.test['image'], labels=self.test['label'], transform=val_transform,image_only=False,transform_with_metadata=True)
